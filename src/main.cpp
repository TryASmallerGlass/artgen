...
static artgen::PixelBuffer render_frame(const artgen::SceneConfig& cfg,
    ...
    artgen::check_render_quality(buf, cfg.algorithm_name);
...
int main(int argc, char* argv[]) {
...
#ifdef ARTGEN_GUI
    // Default to GUI unless --cli is the first argument
    bool cli_mode = (argc > 1 && std::strcmp(argv[1], "--cli") == 0);
    if (!cli_mode)
        return artgen::gui::run();
    // Shift past --cli so the rest of argument parsing is unaffected
    ++argv; --argc;
#endif
    ...
    try {
        ...
        // then replace the filename with an auto-generated stem.
        {
            ...
            std::string stem = artgen::make_output_stem(base.algorithm_name);
            base.output_path = dir + stem + ext;
            artgen::save_settings_json(base, stem);
        }
